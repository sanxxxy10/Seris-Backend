...
router.post("/", verifyToken, async (req, res) => {
  try {
    ...
    try {
      await transporter.sendMail({
        from: `"Seris" <${process.env.EMAIL_USER}>`,
        to: email,
...
// ✅ View only project status and hosting link
...
router.get("/status", verifyToken, async (req, res) => {
  try {
    ...
    const projects = await Project.find({ user: userId }).select(
      "projectName status finalWebsiteLink"
    ).sort({ createdAt: -1 });
...
// ✅ ADMIN: Update project status or hosting link
// Admin: Update project status & final website link
router.put("/admin/update/:id", async (req, res) => {
  try {
    const projectId = req.params.id;
    const { status, finalWebsiteLink } = req.body;

    const updatedProject = await Project.findByIdAndUpdate(
      projectId,
      { $set: { status, finalWebsiteLink } },
      { new: true, runValidators: true }
    );

    res.json({ success: true, projects: updatedProject });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Server Error" });
  }
